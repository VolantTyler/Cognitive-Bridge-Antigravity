...
export const LIVE_GEMINI_MODELS = [
  "gemini-2.5-flash",
...
export const DEFAULT_GEMINI_MODEL = LIVE_GEMINI_MODELS[0];

/** Ids a caller may request. Legacy ids are attempted once, then the live chain. */
export const REQUESTABLE_GEMINI_MODELS = [
  ...LIVE_GEMINI_MODELS,
  "gemini-2.5-pro",
  "gemini-1.5-flash",
  "gemini-1.5-pro",
...
export type ModelFailureKind = "capacity" | "unavailable" | "quota" | "auth" | "other";
...
export function normalizeModelName(name: string | undefined | null): string {
  const cleaned = String(name || "").trim().replace(/^models\//, "");
  return cleaned || DEFAULT_GEMINI_MODEL;
}

export function buildModelChain(requestedModel?: string | null): string[] {
  ...
  push(requested);
  for (const model of LIVE_GEMINI_MODELS) push(model);
  return chain.slice(0, 6);
}

export function isRequestableModel(model: string): boolean {
  return (REQUESTABLE_GEMINI_MODELS as readonly string[]).includes(normalizeModelName(model));
}
...
function includesAny(msg: string, markers: string[]): boolean {
...
export function chainExhaustedMessage(failures: ModelFailure[]): string {
...
export function terminalModelMessage(kind: ModelFailureKind): string {
  if (kind === "quota") {
...
export function httpStatusForKind(kind: ModelFailureKind): number {
  if (kind === "quota") return 429;
  if (kind === "auth") return 403;
  if (kind === "unavailable") return 404;
  if (kind === "capacity") return 503;
  return 500;
}

export function dominantFailureKind(failures: ModelFailure[]): ModelFailureKind {
  const kinds = failures.map((failure) => failure.kind);
  if (kinds.includes("quota")) return "quota";
  if (kinds.includes("auth")) return "auth";
  if (kinds.includes("capacity")) return "capacity";
